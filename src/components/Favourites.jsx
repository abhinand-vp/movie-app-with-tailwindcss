...
function Favourites() {
  ...
  let [searchedItem, setSerachedItem] = useState("")
  let [curGenre, setCurrentgenre] = useState("All Genres");
  let [curRateingOrder, setCurReactingOrder] = useState(0)


  const clickgenre = (gerne) => {
    setCurrentgenre(gerne)
  }
  ...
  const deleteMovies = (id) => {
    let deletedMovies = movies.filter((item) => {
      return item.id != id
    })
  ...
  //search
  let searchedMovies = searchedItem == "" ? movies : movies.filter((movie) => {
    let moviesname = movie.title || movie.name
    let serachedToLOwerCase = searchedItem.toLowerCase()
    return moviesname.toLowerCase().includes(serachedToLOwerCase);
  })

  //filtering
  let filteredMovies =
    curGenre == "All Genres" ? searchedMovies : searchedMovies.filter((movie) => {
      return genreids[movie.genre_ids[0]] == curGenre
    })

  //sorting
  if (curRateingOrder != 0) {
    if (curRateingOrder == 1) {
      filteredMovies = filteredMovies.sort((movieA, movieB) => {
        return movieA.vote_average - movieB.vote_average
      })
    }
    else if (curRateingOrder == -1) {
      filteredMovies = filteredMovies.sort((movieA, movieB) => {
  ...
  return (
    <>
        ...
        {genres.map((genre => {
          ...
          return (
            <button
              ...
              className={`py-1 px-2 bg-gray-400 rounded-lg font-bold text-lg text-white hover:bg-green-600 cursor-pointer ${genre == curGenre ? "bg-green-600" : ""}`}
            > {genre}</button>
          )
        }))}
      ...
      ">
        <input type="text" placeholder='search' value={searchedItem}
          onChange={(e) => setSerachedItem(e.target.value)}
              ...
              <th scope="col" class="px-6 py-4 font-medium text-gray-900">
                <div className='flex'>
                  <img src="https://img.icons8.com/external-those-icons-lineal-those-icons/24/000000/external-up-arrows-those-icons-lineal-those-icons-3.png" class="mr-2 cursor-pointer"
                    onClick={() => setCurReactingOrder(1)}
                  ></img>
                  <div>Rating</div>
                  <img src="https://img.icons8.com/external-those-icons-lineal-those-icons/24/000000/external-down-arrows-those-icons-lineal-those-icons-4.png" class="ml-2 mr-2 cursor-pointer"
                    onClick={() => setCurReactingOrder(-1)}
                  ></img>
                </div>
